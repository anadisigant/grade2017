...
import { getDISCS, getActive } from './curricula.js';

function STORAGE_KEY() { return `bsi_ifba_${getActive()}`; }
...
export function toggle(id) {
  ...
  if (done.has(id)) {
    ...
    function collectDependents(rid) {
      cascade.add(rid);
      getDISCS().filter(x => x.pre.includes(rid)).forEach(x => collectDependents(x.id));
    }
