...
import { getDISCS, getTOTAL_H, getSEM_H, getSEM_LABELS } from './curricula.js';
...
function updateStats() {
  ...
  const pct = Math.round(hDone / getTOTAL_H() * 100);
  document.getElementById('pct-label').textContent = `${pct}%`;
  document.getElementById('pct-hours').textContent = `${hDone} / ${getTOTAL_H()}h`;
...
export function render() {
  ...
  const DISCS = getDISCS();
  const SEM_LABELS = getSEM_LABELS();
  const SEM_H = getSEM_H();

  const bySem = {};
  DISCS.forEach(d => { (bySem[d.sem] ??= []).push(d); });

  for (let s = 1; s <= SEM_LABELS.length; s++) {
    const discs = bySem[s] ?? [];
